...
class SegModel_18(BaseModel):
    def __init__(self, num_classes=None, mode='rl',in_channels=3, backbone='resnet', pretrained=False,
                 output_stride=16, freeze_bn=False, freeze_backbone=False):
        super(SegModel_18, self).__init__()
        ...
        if self.mode == 'seg_train':
            if pretrained:
                self.backbone.load_param('resnet18-5c106cde.pth')
            self.ASSP = ASSP(in_channels=512, output_stride=output_stride)  # 512
